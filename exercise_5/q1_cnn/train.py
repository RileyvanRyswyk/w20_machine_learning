...
def train(num_epochs, batch_size, data_test, initial_lr, cnn_model,
          data_train, data_val, model_file, save_name, train_log_dir):
    """Build and train the network
    ...
    tensorboard_cb = tf.keras.callbacks.TensorBoard(train_log_dir)
    ...
    cnn_model.save(filepath=save_name)
        ...
        #         mean_loss, per_unit_loss = model.training_step(cnn_model, imgs, labels, initial_lr)
        #         tf.summary.scalar("average_loss", mean_loss, step=i)
        #         writer.flush()
        #         # If you want to use the visualization in tensorboard you will need to run merged_summary
        #         # at an appropriate place and assign the result to the summary variable
        #         # use feed_dict={handle: training_handle} as an argument to sess.run in order to specify that
